...
import { getPartidoSenado } from '../queries/queries';
  ...
  render() {
    ...
    return(
      <div className="content">
        <h1>Senadores - {sigla}</h1>
        <Query query={getPartidoSenado} variables={{ sigla }}>
            {({ loading, error, data }) => {
                if (loading) return( <div>Loading ...</div> );
                if (error) return `Error!: ${error}`;
                return data.partidoSenado.senadores.map( obj => {
                    const politico = obj;
